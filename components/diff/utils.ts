  ...
  diffsMap.map((diffs) => {
    ...
    for (let i = 0; i < loopCount; i++) {
      ...
      const chkWords = Boolean(
        getDiffType(diffs[0][0]).match(/added|removed/) && getDiffType(diffs[1][0]).match(/added|removed/)
      );
      ...
      if (child.nodeType === 3) {
        ...
        while (oldContent.length) {
          ...
          const originalCodeDiffLength =
            hasModifiedTag && hasModifiedTag.index ? hasModifiedTag.index : originalCode.length;
