<template>
  ...
  >
    ...
    <template v-if="mode === 'split'">
      <template :key="`line-${index}`" v-for="(line, index) in render">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const rendered = () => {
      if (!lineRef.value || props.meta.height === lineRef.value.offsetHeight)
        return;
    ...
    if (props.scrollOptions) {
      useResizeObserver(
        ...
        useThrottleFn(() => {
          if (
            !lineRef.value ||
            props.meta.height === lineRef.value.offsetHeight
          )
            return;
          emit("setLineHeight", props.meta.index, lineRef.value.offsetHeight);
